...
@routes.get('/media/{filepath}/{name}')
async def give_file(request):
##    app.logger.warning(video_id)
    filepath, filename = request.match_info['filepath'], request.match_info['filename']
    URL = publicUrl+'/'+filepath+'/'+filename
    print(URL)
    return web.FileResponse(URL)

@routes.get("/watch/{filepath}/{filename}")
async def render_image(request):
##    app.logger.warning('Called')
    # video_id = request.args.get('type')
##    app.logger.warning(video_id)
    filepath, filename = request.match_info['filepath'], request.match_info['filename']
    URL = publicUrl+'/'+filepath+'/'+filename
    print(URL)
    log.warning(URL)
    from .htm_code import get_html
    return web.Response(text=get_html(filepath, filename, URL, publicUrl), content_type='text/html')
